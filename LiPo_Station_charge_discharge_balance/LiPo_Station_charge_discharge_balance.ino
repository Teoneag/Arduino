...
String MenuFunctions[6] = {"Charge", "Discharge", "Balance",
                           "Help",   "About",     "Nothing"};
int IMenuFunctions;
...
String HelpMessage =
    "To use this device, press the ok button to select and the + and - buttons "
    "to navigate";
String AboutMessage1 =
    "This device was constructed by Teodor Neagoe as a learning arduino "
    "experiment in the summer of 2021. Please treat it as such!";
String AboutMessage2 =
    "Actually, the real cause of builiding this was the lack of a device that "
    "can put my LiPo acc on storage mode. As buying one is expensive, and I "
    "was planning on not useing one after finishing highshool, I've made this. "
    "Enjoy!";
...
String ReadCells() {
    ...
    CellVoltage[2] =
        analogRead(A2) * 5.0 / 1023 * 3.204 - CellVoltage[1] - CellVoltage[0];
    CellVoltage[3] = analogRead(A3) * 5.0 / 1023 * 4.3 - CellVoltage[2] -
                     CellVoltage[1] - CellVoltage[0];
    NrOfCells = 0;  // This needs improvement: to check nr of cells, u have 2
                    // cases, and I've checked just the first case: Reading of a
                    // non-cell is I. 0 (so it's connected to ground), II.
                    // fluctuateing (so it's not connected to ground)
    for (int j = 0; j < 4 && analogRead(14 + j) > 20; j++) NrOfCells = j + 1;
    String allCells = "Cells not connected";
    if (NrOfCells > 0)
        allCells = "                C1: " + (String)CellVoltage[0],
...
void setup() {
    ...
    lcd.clear();
    String introMessage =
        "This is Teon's accumulator charger, discharger and balancer!          "
        "      ";
    lcd.setCursor(0, 1);
    lcd.print("   (Press ok)");
...
void loop() {
    ...
    if (digitalRead(9) == HIGH)
        IMenuFunctions = (IMenuFunctions <= 0) ? 5 : --IMenuFunctions;
    if (digitalRead(8) == HIGH) {
        delay(300);
        switch (IMenuFunctions) {
            case 0: {
                ...
                lcd.print("Error");
                String errorMessage =
                    "Sorry, an error occurred, please contact a specialist!    "
                    "            ";
                Poz = 0;
                while (Poz != -1) {
                    lcd.setCursor(0, 1);
            ...
            case 1: {
                ...
                while (AllCells != "Stop") {
                    ...
                    if (digitalRead(9) == HIGH)
                        DischargeVoltage =
                            (DischargeVoltage - 0.05 < 0)
                                ? 3.85
                                : DischargeVoltage -
                                      0.05;  // A precisioun variable and
                                             // setting menu would be a future
                                             // ideea
                    if (digitalRead(10) == HIGH)
                        DischargeVoltage = (MinVoltage < DischargeVoltage + 0.05
                                                ? 0
                ...
                while (Ok < 300 && Poz != -1) {
                    ...
                    for (int j = 0; j < 250; j++) {
                        ...
                        if (MinVoltage < DischargeVoltage)
                            Ok++;
                        else
                            Ok = 0;
                        delay(1);
